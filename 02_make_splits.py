# build flow-level train / val / test_known / test_unknown splits
# from preprocessed_sorted already materialised in iot23_3.duckdb.
#
# pipeline:
# 1. pull all relevant flows from duckdb cli -> pandas dataframe
# 2. encode the 3 categorical columns (conn_state, history, service) with
#    integer vocabularies built from the training data only (no leakage).
# 3. numeric sentinel: uses -1 for features with semantic missingness
#    (zeek fields) and 0 for packet counters. the preprocessed view already
#    applied coalesce, so nulls should be gone but we add a fillna(-1)
#    safety net for any residual nans in numeric cols.
# 4. undersample each known class to SAMPLES_PER_CLASS for balance.
# 5. stratified 70/15/15 split (train/val/test_known).
# 6. unknown primary + rare classes -> test_unknown (all available rows).
# 7. save vocab.json + four split parquets.
