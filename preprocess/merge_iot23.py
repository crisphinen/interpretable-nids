# iot-23 zeek + dpkt feature merger, duckdb-native version.
# duckdb reads both csv files directly from disk; no pandas, no oom.
# all streaming, spilling, and chunking is handled internally by duckdb.
#
# zeek conn.log.labeled files are non-standard (tab-separated, # headers).
# the last field is space-joined, so we preprocess them to clean temp csvs first.
#
# usage:
#   python merge_zeek_dpkt.py              - skip already-merged scenarios
#   python merge_zeek_dpkt.py --reset      - delete and reprocess all scenarios
...
# stream zeek logs line by line, filter broadcasts, write clean csv.
# returns (total_raw, total_clean, label_counts_dict).
# uses negligible ram - processes one line at a time.
...
# duckdb join sql - reads both files directly from disk
JOIN_SQL = """
COPY (
    SELECT
        z.src_ip,
        z.dst_ip,
        TRY_CAST(z.src_port AS INTEGER)         AS src_port,
        TRY_CAST(z.dst_port AS INTEGER)         AS dst_port,
        z.protocol_zeek                         AS protocol,
        TRY_CAST(z.ts_zeek   AS DOUBLE)         AS ts,
        TRY_CAST(z.duration  AS DOUBLE)         AS duration,
        z.conn_state,
        z.history,
        z.local_orig,
        z.local_resp,
        TRY_CAST(z.missed_bytes   AS DOUBLE)    AS missed_bytes,
        TRY_CAST(z.orig_bytes     AS DOUBLE)    AS orig_bytes,
        TRY_CAST(z.orig_ip_bytes  AS DOUBLE)    AS orig_ip_bytes,
        TRY_CAST(z.orig_pkts      AS DOUBLE)    AS orig_pkts,
        z.service,
        TRY_CAST(z.resp_bytes     AS DOUBLE)    AS resp_bytes,
        TRY_CAST(z.resp_ip_bytes  AS DOUBLE)    AS resp_ip_bytes,
        TRY_CAST(z.resp_pkts      AS DOUBLE)    AS resp_pkts,
        z.tunnel_parents,
        z.label,
        z.detailed_label,
        d.Header_Length,
        ...
        d.Srate, d.Drate, d.Magnitude, d.Radius, d.Weight, d.Covariance,
        d.ts_start                              AS dpkt_ts_start,
        TRY_CAST(z.orig_bytes AS DOUBLE)
            / (TRY_CAST(z.resp_bytes AS DOUBLE) + 1)    AS byte_ratio,
        TRY_CAST(z.orig_pkts AS DOUBLE)
            / (TRY_CAST(z.duration AS DOUBLE) + 1)      AS orig_pkt_rate,
        TRY_CAST(z.orig_ip_bytes AS DOUBLE)
            / (TRY_CAST(z.duration AS DOUBLE) + 1)      AS orig_byte_rate,
        TRY_CAST(z.orig_bytes AS DOUBLE)
            - TRY_CAST(z.resp_bytes AS DOUBLE)           AS direction
    FROM read_csv_auto('{zeek_csv}',
                  header=true,
                  all_varchar=true,
                  ignore_errors=true) z
    INNER JOIN read_csv_auto('{dpkt_csv}',
                        header=true,
                        all_varchar=true,
                        ignore_errors=true) d
        ON  z.src_ip = d.src_ip
        AND z.dst_ip = d.dst_ip
        AND TRY_CAST(z.src_port AS INTEGER) = TRY_CAST(d.src_port AS INTEGER)
        AND TRY_CAST(z.dst_port AS INTEGER) = TRY_CAST(d.dst_port AS INTEGER)
        AND TRY_CAST(d.ts_start AS DOUBLE)
              BETWEEN TRY_CAST(z.ts_zeek AS DOUBLE)
                  AND TRY_CAST(z.ts_zeek AS DOUBLE)
                      + COALESCE(TRY_CAST(z.duration AS DOUBLE), 0) + 1
) TO '{out_csv}' (HEADER, DELIMITER ',');
"""


# per-scenario merge
def merge_scenario(capture_dir, con, reset=False):
    name     = os.path.basename(capture_dir)
    ...
    if os.path.exists(out_path):
        if reset:
            os.remove(out_path)
            log(f"[{name}] RESET - deleted existing merged CSV")
        else:
    ...
    try:
        log(f"[{name}] Step 1/2: Preprocessing Zeek log to temp CSV...")
        ...
        log(f"[{name}] Step 2/2: DuckDB join (reading files from disk)...")
        sql = JOIN_SQL.format(
    ...
    if not os.path.exists(tmp_out):
        ...
        merged_rows = con.execute(
            f"SELECT COUNT(*) FROM read_csv_auto('{os.path.abspath(tmp_out)}', header=true)"
        ).fetchone()[0]
    ...
    # label stats from merged output - duckdb reads from disk, no pandas load
    merged_benign = merged_malicious = merged_unique = 0
    if merged_rows > 0:
        lbl_df = con.execute(
            f"SELECT label, COUNT(*) AS cnt "
            f"FROM read_csv_auto('{os.path.abspath(out_path)}', header=true) "
            f"GROUP BY label"
        ).df()
    ...
    dpkt_rows = con.execute(
        f"SELECT COUNT(*) FROM read_csv_auto('{os.path.abspath(dpkt_path)}', header=true)"
    ).fetchone()[0]
...
def main():
    ...
    log(f"Found {len(all_captures)} capture directories")
    ...
    for i, capture_dir in enumerate(all_captures, 1):
        ...
        stats = merge_scenario(capture_dir, con, reset=args.reset)
