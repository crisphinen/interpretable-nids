...
\begin{enumerate}
  \item \textbf{Main finding---open-set detectability is not reliably governed by any tunable or measurable factor.} Across two IoT benchmarks, six architectures and five seeds, OOD detectability is not controlled by concept-fidelity regularisation ($\gamma$; effect $\le0.07$, sign flipping across five dataset/split combinations, Table~\ref{tab:gamma_splits}) and is not predicted by any known-class geometry statistic: within-class variance, class separability, and even closed-set accuracy all \emph{reverse} across architectures (a Simpson's paradox, Section~\ref{sec:ksweep}). Single-seed effects, such as a $0.06$-AUROC ``task-coupling'', do not replicate. We offer this as a practical evaluation standard---open-set claims on interpretable NIDS need multi-seed, multi-architecture, multi-split validation, not one favourable configuration.
  \item \textbf{Scorer choice is dataset-contingent and consequential.} The best OOD scorer flips between datasets across all six architectures---per-class Mahalanobis on CTU-IoT-23, where a logit-space energy score is below chance, versus energy on CIC-IoT-2023---so the detector must be matched to the deployment, not chosen a priori (Tables~\ref{tab:scorer},~\ref{tab:scorer_arch}).
  \item \textbf{Interpretability at zero accuracy cost.} Jointly trained CBMs and NeSy-NIDS match an unconstrained MLP on known-class F1 (within $0.002$ over five seeds on both datasets) under open-set evaluation.
  \item \textbf{Binary, auditable rules.} NeSy-NIDS learns rule thresholds through $k$-annealed sigmoid gates and binarises the activations at inference, giving exactly binary, syntactically checkable if-then rules at no accuracy cost; a penalty on the gate $\alpha$ raises rule reliance across most of its range at no cost in F1 or OOD AUROC.
\end{enumerate}
...
scoring~\cite{lee2018simple}, and neural/rule hybridisation. The contribution is
therefore not a new detector or a state-of-the-art result; it is a controlled,
multi-seed, multi-architecture open-set study whose \emph{primary result is
negative}---open-set detectability in these models is not reliably governed by
regularisation strength or by any known-class geometry statistic, and a plausible
single-seed effect does not replicate. We intend this as methodological guidance for
building and evaluating interpretable IoT NIDS, in line with calls for more careful
...
best OOD representation on CTU-IoT-23 even under the harder split, with
concept-fidelity $\gamma$ again showing no consistent bearing on detectability.
...
\textbf{Scope of the negative result.} Because the caution rests on two datasets we
state its reach precisely. What we expect to \emph{transfer} is the structural claim
and the evaluation lesson: since OOD detectability is set by where unknown families
land relative to the known-class model---a quantity orthogonal to known-class
properties---interpretable NIDS scored by per-class Mahalanobis or related
activation-space detectors, under family-holdout open-set evaluation, should show the
same decoupling of closed-set quality from open-set detection and the same seed-,
architecture- and split-sensitivity. What we \emph{do not} claim to generalise is any
particular direction: that CTU-like corpora favour activation-space scorers and
CIC-like corpora favour a logit-space energy score, that a given metric's sign is
fixed, or that \emph{no} known-class predictor exists---two datasets show that the
natural candidates fail here, not that none can succeed anywhere. The finding also
need not hold for packet- or graph-level representations, learned rather than
hand-authored concept vocabularies, or OOD scorers beyond the ones we test.
Expanding the claim to ``no known-class predictor governs open-set detectability in
IoT NIDS'' has a concrete minimum: replicate the geometry$\times$architecture
factorial on at least two further IoT corpora with distinct feature
schemas---one Zeek-style, so the CTU vocabulary transfers unchanged, and one
packet-level, requiring a fresh vocabulary---each with its own family-holdout splits,
and test whether the sign reversals persist. A metric that predicts detectability
consistently across four or more datasets would weaken the negative; persistent
reversals would graduate the caution from an existence proof to a general standard.

\section{Conclusion}

We evaluated CBMs and NeSy-NIDS as interpretable open-set NIDS on two IoT
datasets under one multi-seed, multi-architecture protocol. Both match an
unconstrained MLP on known-class F1 ($\pm0.002$) while exposing a representation
an analyst can read---interpretability at no accuracy cost. Our central result,
however, is a caution: open-set detectability in these models is \emph{not}
reliably governed by any factor one would tune or measure. Concept-fidelity
regularisation has no consistent effect (its small influence changes sign across
five dataset/split combinations); no known-class geometry statistic predicts it
(within-class variance, separability and closed-set accuracy all reverse across
architectures); and single-seed effects such as a ``task-coupling'' penalty do not
replicate. Which scorer works is itself dataset-contingent---activation-space on
CTU-IoT-23, logit-space energy on CIC-IoT-2023---and cannot be chosen a priori.
Open-set claims about interpretable NIDS therefore need multi-seed,
multi-architecture, multi-split validation rather than one favourable
