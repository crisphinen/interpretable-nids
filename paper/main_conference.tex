...
\author{Crisphine~Macharia~Ngari and~Ning~Weng%
...
\begin{enumerate}
  ...
  \item \textbf{Scorer--representation matching}: OOD detectability depends on the geometry of the representation more than on the scoring rule applied to it. On well-separated rule activations (CTU) per-class Mahalanobis and centroid-based Hamming and Jaccard distances all work; on near-deterministic activations (CIC) every activation-space scorer is weak and only a logit-space energy score restores detection.
...
scorer choice. On the well-separated CTU rule space every activation-space scorer works
(NeSy-NIDS with per-class Mahalanobis: $0.899$); on the near-deterministic
CIC space every activation-space scorer fails ($0.615$) and a logit-space
energy score ($0.749$) restores detection. Concept-fidelity regularisation
strength, in contrast, has no consistent effect on OOD AUROC: its small
influence changes sign across datasets and splits, and a single-seed
``task-coupling'' effect does not survive replication.
