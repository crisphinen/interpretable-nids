...
\begin{abstract}
Machine-learning intrusion detection systems (NIDS) for IoT networks face two obstacles to deployment: operators cannot see why a flow was flagged, and models trained on a closed set of attack families misclassify novel ones (the open-set problem). We study two interpretable architectures under one open-set protocol. Concept Bottleneck Models (CBMs) route predictions through a layer of human-defined traffic concepts and allow test-time intervention. Neuro-Symbolic NIDS (NeSy-NIDS) turns expert attack signatures into threshold rules with learnable thresholds, trained with $k$-annealed sigmoid gates and binarised at inference, alongside a neural fallback blended through a learnable gate $\alpha$. On CTU-IoT-23 and CIC-IoT-2023, jointly trained CBMs and NeSy-NIDS match an unconstrained MLP on known-class F1 (within $\pm0.002$ over five seeds), so interpretability costs no accuracy. We then ask what governs out-of-distribution (OOD) detection. The representation decides whether a scoring rule can work: on the well-separated rule activations of CTU-IoT-23, per-class Mahalanobis distance~\cite{lee2018simple} and simpler centroid distances all reach $0.86$--$0.90$ AUROC (NeSy-NIDS $0.899\pm0.013$), while on the near-deterministic activations of CIC-IoT-2023 no activation-space scorer exceeds $0.64$ and only a logit-space energy score ($0.749\pm0.014$) restores detection. Concept-fidelity regularisation strength, by contrast, has no consistent effect: its small influence on OOD AUROC changes sign between datasets and between splits of the same dataset, and a $0.06$-AUROC ``task-coupling'' effect seen at one seed does not survive replication. Representation geometry and scorer choice, not regularisation strength, decide open-set detectability in these models, and single-seed open-set results on interpretable NIDS need replication before they are reported. NeSy-NIDS additionally yields exactly binary rule activations for audit and a gate $\alpha$ that raises rule reliance at no F1 cost.
\end{abstract}
...
test (test strictly follows train; the held-out unknown families are
unchanged). Under this harder protocol (three seeds) known-class F1
