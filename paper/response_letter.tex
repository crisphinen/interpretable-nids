...
\textbf{Manuscript:} Towards Trustworthy IoT Intrusion Detection: What Governs
Open-Set Recognition in Interpretable Models\\[2pt]
...
\begin{itemize}
  ...
  \item Added a \textbf{controlled within-dataset test} of the representation-geometry
  claim (Section~IV-C, Fig.~5): a training-time annealing sweep ($k_T\in\{2,5,10,20\}$,
  five seeds) that moves the interpretable representation's within-class variance at
  constant known-class F1. On CIC-IoT-2023 detectability follows the induced variance
  in the predicted direction; on CTU-IoT-23, where the variance does not move, it is
  unchanged---turning the previously between-dataset association into a within-dataset
  manipulation, while making explicit that annealing moves detectability only within a
  dataset's regime.
\end{itemize}
