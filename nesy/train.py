...
K_WARMUP      = 30    # epochs to ramp k from K_INIT → K_FINAL
...
def train_nesy(model, loader_tr, loader_val, device, label,
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        for xb, yb in loader_tr:
            ...
            # α-regularisation: penalise low rule weight, pushing model to rely
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            if hasattr(model, 'gate'):
                gate_str = f"  α={model.get_gate_value():.3f}"
...
def main():
    ...
    parser.add_argument("--lambda_alpha", type=float, default=0.0,
                        help="Penalty on (1-α)^2 to push model toward higher rule weight.")
