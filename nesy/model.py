...
class RuleTemplate:
    ...
    condition_types: List[Literal['gt','lt']]  # 'gt' = x>θ, 'lt' = x<θ
...
CTU_RULES = [
    ...
    RuleTemplate(
        ...
        init_thresholds=[1.5, 5.0, 0.1, 5.0],  # IAT∈(1.5,5), Rate∈(0.1,5)
    ),
...
class DiffRule(nn.Module):
    """One differentiable logical rule; score ∈ (0,1) measures condition match."""
...
class NeSyNIDS(nn.Module):
    ...
    def __init__(
        ...
        # Gating: global scalar α ∈ (0,1) or per-sample via small network
        if global_gate:
            ...
            # Per-sample gate: small network predicts α from input
