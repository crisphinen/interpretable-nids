...
def rule_editing_experiment(model, X: torch.Tensor, y: torch.Tensor, k: float = K_EVAL):
    """Analogous to CBM concept interventions: force each rule to 1/0 and measure accuracy delta."""
    # Baseline accuracy
...
def print_rule_summary(crispness, selectivity, editing, id_to_label):
    ...
    # Crispness table
    print(f"\n  {'Rule':<30}  {'Crisp':>7}  {'ΔAcc(hi)':>9}  {'ΔAcc(lo)':>9}")
    print(f"  {'-'*62}")
...
def main():
    ...
    if not np.isnan(gate_val):
        print(f"  Gate α (rule weight): {gate_val:.4f}")
    ...
    if hasattr(model, 'rule_bank'):
        ...
        for rule in model.rule_bank.rules:
            ...
            theta_str = ", ".join(f"feat[{fi}]{ct}θ={th:.3f}"
